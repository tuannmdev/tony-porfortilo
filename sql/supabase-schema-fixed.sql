-- =====================================================
-- TONY'S PORTFOLIO - SUPABASE DATABASE SCHEMA (FIXED)
-- =====================================================
-- This is the corrected version without immutable generated columns
-- =====================================================
...
-- =====================================================
-- TABLE: experience (WITHOUT GENERATED COLUMNS)
-- =====================================================
...
-- Note: is_current and duration_months will be computed in the application layer
...
-- IMPORTANT: Replace 'admin@yourportfolio.com' with your actual admin email after creating the auth user
...
-- Note: This function uses CURRENT_DATE which makes it non-deterministic,
-- but it's OK for a function (just not for generated columns)
CREATE OR REPLACE FUNCTION calculate_experience_duration(start_date DATE, end_date DATE)
RETURNS INTEGER AS $$
BEGIN
  IF end_date IS NULL THEN
    RETURN EXTRACT(YEAR FROM AGE(CURRENT_DATE, start_date))::INTEGER * 12 +
           EXTRACT(MONTH FROM AGE(CURRENT_DATE, start_date))::INTEGER;
  ELSE
    RETURN EXTRACT(YEAR FROM AGE(end_date, start_date))::INTEGER * 12 +
...
BEGIN
  ...
  RAISE NOTICE '   - company-logos (public)';
  RAISE NOTICE '   - documents (public)';
  RAISE NOTICE '';
  RAISE NOTICE '2. Set up Supabase Auth:';
  RAISE NOTICE '   - Enable Email provider';
  RAISE NOTICE '   - Create admin user with email: admin@yourportfolio.com';
  RAISE NOTICE '';
  RAISE NOTICE '3. Update RLS policies with your admin email';
  RAISE NOTICE '';
  RAISE NOTICE '4. Copy Supabase credentials to .env.local:';
